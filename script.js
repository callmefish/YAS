  ...
  stream => {
    ...
    recordBtn.onclick = () => {
      if (mediaRecorder.state === "recording") {
        ...
        console.log("End of reconding");
      } else {
        mediaRecorder.start();
        console.log("Reconding...");
...
const relPos = (x, y) => {
  // calculate from mouse/touch position to svg coordinate
...
const SoundMachine = {
  ...
  // is muted, it can be reused, avoiding memory issues.
...
svg.addEventListener("mousedown", e => {
  if (touchNoises.length > 0) {
    ...
    // this would result in a never-ending beeeeeeeep on mobile
    return;
  }
  if (e.button === 1) {
